...
bobliew::ByteArray::ptr ZlibStream::getByteArray() {
...
int ZlibStream::init(Type type, int level, int window_bits,
    ...
    BOBLIEW_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    BOBLIEW_ASSERT((window_bits >= 8 && window_bits <= 15));
    BOBLIEW_ASSERT((memlevel >= 1 && memlevel <= 9));
