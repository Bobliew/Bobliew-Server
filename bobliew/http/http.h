#ifndef __BOBLIEW_HTTP_H__
#define __BOBLIEW_HTTP_H__
...
#define HTTP_STATUS_MAP(XX)                                                 \
    ...
    XX(307, TEMPORARY_REDIRECT,             Temporary Redicrect)            \
    ...
    XX(408, PROXY_AUTHENTICATION_REQUIRED,  Proxy Authentication Required)  \
    ...
    XX(451, UNAVAILABLE_FOR_LEGAL_RESONS,   Unavailable For Legal Reason)   \
...
public:
    ...
    // MAP结构 CaseInsensitveLess是Compare参数
    typedef std::map<std::string, std::string, CaseInsensitiveLess> MapType;
    //构造函数，version版本，close是否keepalive
    ...
    //是否是websocket
    bool isWebsocket(bool v) { return m_websocket;}
    //设置是否websocket
    ...
    template<class T>
    T checkGetCookieAs(const std::string& key, T& val, const T& def = T()) {
        initCookies();
...
public:
    ...
    uint8_t getVersion() const { return m_version;}
    const std::string& getBody() const { return m_body;}
    const std::string& getReasion() const { return m_reason;}
    const MapType& getHeaders() const { return m_headers;}
