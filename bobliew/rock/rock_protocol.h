#ifndef __BOBLIEW_ROCK_PROTOCOL_H__
#define __BOBLIEW_ROCK_PROTOCOL_H__
...
public:
    ...
    virtual bool parserFromByteArray(ByteArray::ptr bytearray);

    template<class T>
    std::shared_ptr<T> getAsPB() const {
        try {
            std::shared_ptr<T> data(new T);
            if(data->ParserFromString(m_body)) {
                return data;
            }
        } catch (...) {
        }
...
public:
    ...
    virtual Message::ptr parserFrom(Stream::ptr stream) override;
