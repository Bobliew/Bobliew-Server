...
HttpConnection::~HttpConnection() {
    BOBLIEW_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            BOBLIEW_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
    ...
    if(!body.empty()) {
        ...
        BOBLIEW_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
            << " size=" << body.size();
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req,
    ...
    if(!addr) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_HOST,
                                            nullptr, "invalid host:" + addr->toString());
    }
    ...
    if(rt < 0) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SOCKET_ERROR,
                                            ...
                                            + std::to_string(errno)+" errstr=" +
                                            std::string(strerror(errno)));
    }
    auto rsp = conn->recvResponse();
    if(!rsp) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT,
                                            nullptr, "recv response timeout: "+
                                            addr->toString() + " timeout_ms" +
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri,
    ...
    if(!turi) {
        BOBLIEW_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
    return std::make_shared<HttpConnectionPool>(turi->getHost(), vhost,
                                                turi->getPort(), turi->getScheme() == "https",
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    if(!ptr) {
        ...
        if(!addr) {
            BOBLIEW_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = m_isHttps ? SSLSocket::CreateTCP(addr) : Socket::CreateTCP(addr);
        if(!sock) {
            BOBLIEW_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            BOBLIEW_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
