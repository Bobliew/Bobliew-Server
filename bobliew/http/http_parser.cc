...
static bobliew::ConfigVar<uint64_t>::ptr g_http_request_buffer_size = 
    bobliew::Config::Lookup("http.request.buffer_size", (uint64_t)(4 * 1024)
                            ,"http request buffer size");

static bobliew::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
    bobliew::Config::Lookup("http.request.max_body_size"
                            ,(uint64_t)(64 * 1024 * 1024), "http request max body size");

static bobliew::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    bobliew::Config::Lookup("http.response.buffer_size"
                            ,(uint64_t)(4 * 1024), "http response buffer size");

static bobliew::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
    bobliew::Config::Lookup("http.response.max_body_size"
...
void on_request_method(void *data, const char *at, size_t length) {
    ...
    if(m == HttpMethod::INVALID_METHOD) {
        BOBLIEW_LOG_WARN(g_logger) << "invalid http request method: "
...
void on_request_version(void *data, const char *at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        BOBLIEW_LOG_WARN(g_logger) << "invalid http request version: "
        << std::string(at, length);
...
,const char *value, size_t vlen) {
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
    if(flen == 0) {
        BOBLIEW_LOG_WARN(g_logger) << "invalid http requests field length ==0";
        return;
    }
...
void on_response_version(void *data, const char *at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        BOBLIEW_LOG_WARN(g_logger) << "invalid http response version: "
        <<std::string(at, length);
        parser->setError(1001);
        return;
    }
...
,const char *value, size_t vlen) {
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
    if(flen == 0) {
        BOBLIEW_LOG_WARN(g_logger) << "invalid http response field length ==0";
        return;
    }
    parser->getData()->setHeader(std::string(field, flen), std::string(value, vlen));
}
...
HttpResponseParser::HttpResponseParser() :m_error(0) {
    m_data.reset(new bobliew::http::HttpResponse);
